# Referencja data-testid dla testów E2E

## Scenariusz: Edycja tytu?u quizu

### Przegl?d
Ten dokument zawiera list? wszystkich atrybutów `data-testid` dodanych do komponentów zwi?zanych ze scenariuszem edycji tytu?u quizu i zapisywania zmian.

### Mapa kroków scenariusza z data-testid

#### 1. Lista quizów (Dashboard)
| Element | data-testid | Lokalizacja | Opis |
|---------|-------------|-------------|------|
| Kontener listy quizów | `quiz-list` | `QuizList.tsx` | G?ówny kontener zawieraj?cy wszystkie karty quizów |
...
| Przycisk "Powró? do listy quizów" | `return-to-quiz-list-button` | `QuizEditView.tsx` | Przycisk powrotu do dashboardu |
...
test('Edycja tytu?u quizu', async () => {
  ...
  // 8. Kliknij przycisk "Powró? do listy quizów"
...
1. **Umieszczanie atrybutów**: Atrybuty `data-testid` s? umieszczane bezpo?rednio na rzeczywistych elementach DOM (button, input, div), a nie na komponentach React.

2. **Nazewnictwo**: 
   - U?ywaj kebab-case
   - Nazwa powinna opisywa? funkcj? elementu lub akcj?
   - Dla przycisków: `{akcja}-button`
   - Dla kontenerów: `{nazwa}-bar`, `{nazwa}-list`
   - Dla pól tekstowych: `{nazwa}-input`
